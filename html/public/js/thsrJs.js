	...
	jQuery.ajax({
		...
		success: function(Data){
			if(Data.type=='success'){
				if(queStr=='stationNow'){
					...
					if(Data.DS=='' && Data.DN==''){
						...
						if(Data.DN!=''){
							...
							dn.forEach(d =>{
								togo += '<div class="row" style="padding-bottom:3vh;"><div class="col"></div>';
								togo += '<div class="col-10" align=left onclick=TraStatus("'+d.TrainNo+'") style="background:#fff;padding-top:.3vh;padding-bottom:.3vh;border-color:#d0d0d0;border-width:0.2vh;border-style:solid;border-radius:0.5vh;")><a href="#" style="text-decoration:none;">'+d.ArrivalTime+'&emsp;開往: '+d.DestinationStationNameTW+'&nbsp;&nbsp;車次: '+d.TrainNo+'<br>'+d.TrainTypeNameTW+'<div id='+d.TrainNo+' style="margin-top:.3vh"></div></div><div class="col"></div></a></div>';
							});
						}
						else{
						...
						if(Data.DS!=''){
							...
							ds.forEach(d =>{
								toback += '<div class="row" style="padding-bottom:3vh;"><div class="col"></div>';
				...
				let errTe = '';
				errTe += dom+go+noDS+noDC+Data.errT+noDE+endDiv+back+noDS+noDC+Data.errT+noDE+endDiv+endDiv;
				if(requStation==queStr){
					$("#Mline").empty();
					$("#Mline").append(errTe);
				}
...
function TraStatus(TrainNo){
	...
	jQuery.ajax({
		...
		success: function(Data){
			if(Data.type=='success'){
				$("#"+TrainNo).empty();
				$("#"+TrainNo).append(Data.TrainStatusD.statusD);
			}
			else{
				$("#"+TrainNo).empty();
				$("#"+TrainNo).append(Data.errT);
			}
		},
		error: function(xhr, ajaxOptions, thrownError){
				$("#"+TrainNo).empty();
